...
import 'dashboard_page_new.dart';
...
class _AllEntriesPageState extends State<AllEntriesPage>
  ...
  Future<void> _exportCsv() async {
    try {
      final path = await appStore.exportCSV();
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('CSV salvat: $path')),
      );
    } catch (err) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Export esuat: $err')),
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final byUser = <String, List<PontajEntry>>{};
    for (final e in entries) {
      byUser.putIfAbsent(e.user, () => []).add(e);
    }
    final groupedEntries = byUser.entries.toList()
